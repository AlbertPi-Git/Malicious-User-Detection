...
function  [trust_table,If_Urgent]=Dead_Reckon(x1,Buffer,cur_clk,dt,F,B,ori_u1,Var_mea,Var_self,gt_trust)
    ...
    % Res_index=2.5;
    % min_var_index=mod(round(1000*rand),total_vehicle-1)+1;
    % Expec_ResSE=2*Var_mea;
    % Var_ResSE=2*Expec_ResSE^2/buffer_size;
    
    % ResSE=zeros(2,total_vehicle-1);
    % for i=1:total_vehicle-1
    %     temp=Buffer{i}([1,3],:)-Buffer{min_var_index}([1,3],:);
    %     ResSE(:,i)=sum(temp.^2,2)/buffer_size;
    % end
    % ResSE(:,min_var_index)=[];
    % Freq_Test=false;
    % for i=1:total_vehicle-2
    %     if(abs(ResSE(1,i)-Expec_ResSE)>Res_index*sqrt(Var_ResSE)||abs(ResSE(2,i)-Expec_ResSE)>Res_index*sqrt(Var_ResSE))
    %         Freq_Test=true;
    %     end
    % end

    
    % ResSE_trust_table=ones(1,total_vehicle-1);
    % if(Freq_Test)
    %     spec_cluster_id=kmeans(ResSE',2);
    %     spec_honest_id=mode(spec_cluster_id);
    %     for j=1:total_vehicle-2
    %         if(spec_cluster_id(j)~=spec_honest_id) 
    %             ResSE_trust_table(j+(j>=min_var_index))=0;
    %         end
    %     end
    % else
    %     ResSE_trust_table=ones(1,total_vehicle-1);
    ...
    % trust_table=ResSE_trust_table;
    % trust_table=Seq_trust_table{1}&ResSE_trust_table;
