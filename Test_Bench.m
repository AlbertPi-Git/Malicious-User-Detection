...
var_self=9; %Variance of self-observation
var_mea=9;  %Variance of observations from other vehicles
mal_var_coef=1; %variance of malicious observaion/variance of normal observation
...
collu_design_mal_devi_coef=5/3; %deviation in cooradinated trajectory attack=collu_design_mal_devi_coef*sqrt(var_mea)
buffer_size=16; %Buffer size for sequential detection
randAver_times = 15; %How many times will the whole simulation be execeuted to get average performance results
...
if strcmp(test_mode,'oneshot')
    ...
    randAver_times = 10; %No need to average
...
if strcmp(test_mode,'varying_mali') % Need to set num_minvehicle and num_vehicle as the same
    ...
    legend('RMSE curve of Ground Truth','RMSE curve of LMS','RMSE curve of MMAE','RMSE curve of DMMSD(Proposed)');
%     legend('RMSE curve of Ground Truth','RMSE curve of LMS','RMSE curve of MMAE','RMSE curve of DMMSD(Proposed)','RMSE curve of MRED(Proposed)');
    xlabel('Number of malicious vehicles','fontsize',24);
    ylabel('RMSE (m)','fontsize',24);
...
if strcmp(test_mode,'collu_design_devi_sweep')
    ...
    collu_design_mal_devi_coef=1.2:0.2:2.8;     %Change malicious deviation coefficient
...
%     plot(collu_design_mal_devi_coef.*sqrt(var_mea),FPR_Devi(3,:),'-^','Linewidth',2.5,'Markersize',9);
    ylim([0,1]);
    xlabel('Malicious deviation in Y direction (m)','Fontsize',24);
    ylabel('TPR & FPR','Fontsize',24);
    legend('TPR of SeqMMSE','TPR of DMMSD(Proposed)','FPR of SeqMMSE','FPR of DMMSD(Proposed)');
...
if strcmp(test_mode,'collu_design_var_sweep')
    space_attack_mode='collu';
    time_attack_mode = 'long_design';
    collu_design_mal_devi_coef=1.8;     %Fix malicious deviation coefficient
    mal_var_coef=0.4:0.2:2;       %Change malicious variance coefficient
...
%         [~,TPR_Var(3,i),FPR_Var(3,i),TNR_Var(3,i),FNR_Var(3,i)]=CoopTracking_MalDetection(var_self,var_mea,mal_var_coef(i),num_vehicle,num_minvehi,num_malicious,'MRED',buffer_size,space_attack_mode,time_attack_mode,randAver_times,collu_design_mal_devi_coef,1,test_mode);
    ...
    grid on;
    box on;
    plot(mal_var_coef*var_mea,TPR_Var(1,:),'--o','Linewidth',2.5,'Markersize',8);
    plot(mal_var_coef*var_mea,TPR_Var(2,:),'-.*','Linewidth',2.5,'Markersize',12);
%     plot(mal_var_coef*var_mea,TPR_Var(3,:),'-x','Linewidth',2.5,'Markersize',12);
    plot(mal_var_coef*var_mea,FPR_Var(1,:),'--s','Linewidth',2.5,'Markersize',9);
    plot(mal_var_coef*var_mea,FPR_Var(2,:),'-.d','Linewidth',2.5,'Markersize',9);
%     plot(mal_var_coef*var_mea,FPR_Var(3,:),'-^','Linewidth',2.5,'Markersize',9);
    xticks([0.4:0.2:2]*9);
    xlim([0.4*9,2*9]);
    ylim([0,1]);
    xlabel('Variance of malicious observations (m^2)','Fontsize',24);
    ylabel('TPR & FPR','Fontsize',24);
    legend('TPR of SeqMMSE','TPR of DMMSD(Proposed)','FPR of SeqMMSE','FPR of DMMSD(Proposed)');
%     legend('TPR of SeqMMSE','TPR of DMMSD(Proposed)','TPR of MRED(Proposed)','FPR of SeqMMSE','FPR of DMMSD(Proposed)','FPR of MRED(Proposed)');
